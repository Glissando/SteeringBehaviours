...
public class SteeringBehaviours : MonoBehaviour{
	...
	public Vector3 Avoid(float avoidanceForce){
		Ray ray = new Ray(transform.position,transform.forward);
		RaycastHit hit;
		Physics.Raycast(ray,out hit,Mathf.Infinity);
		return (hit.point-hit.collider.bounds.center).normalized * avoidanceForce;
	}
	...
	public Vector3 Arrive(Transform target, float maxDistance){
		float dRatio = maxDistance/Vector3.Distance(transform.position,target.position);
		return (dRatio > 1) ? Seek(target)/dRatio : Seek(target);
	}
	
	public Vector3 Arrive(Vector3 target,float maxDistance){
		float dRatio = maxDistance/Vector3.Distance(transform.position,target);
		return (dRatio > 1) ? Seek(target)/dRatio : Seek(target);
	}
